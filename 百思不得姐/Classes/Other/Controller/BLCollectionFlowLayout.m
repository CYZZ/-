...
@implementation BLCollectionFlowLayout
{
    NSIndexPath *mainIndexPath;
    NSIndexPath *movingIndexPath;
    CGFloat previousOffet;
    CGFloat difference;
}
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self applyTransforToLayoutAttributes:attributes];
    
    return attributes;
}

- (void)applyTransforToLayoutAttributes:(UICollectionViewLayoutAttributes *)attribute
{
    if (attribute.indexPath.section == mainIndexPath.section) {
        ...
        attribute.transform3D = [self transformFormVIew:cell];
    }
}
...
- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect
{
    ...
    if (cellIndices.count == 0) {
        ...
        if (firstIndexPath == mainIndexPath) {
            movingIndexPath = cellIndices[1];
        }else{
            movingIndexPath = cellIndices.firstObject;
...
- (CATransform3D)transformFormVIew:(UIView *)view
{
